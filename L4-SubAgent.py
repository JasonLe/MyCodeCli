...
def spawn_subagent(prompt: str):
    """创建子Agent去执行任务，最后返回summary后的结果"""

    sub_agent_llm = ChatNVIDIA(
        model="qwen/qwen3.5-122b-a10b",
        api_key=os.getenv("NVIDIA_API_KEY"),
        temperature=0.6,
        top_p=0.95,
    ...
    sub_messages: List[BaseMessage] = [
        SystemMessage(
            content=f"您是 {WORK_DIR} 的一名子Agent。优先使用工具完成任务而非文字描述。执行完最后返回结果时做总结"),
        HumanMessage(content=prompt),
    ]

    while True:
        response = sub_agent_llm.invoke(sub_messages)
        ...
        for tool_call in response.tool_calls:
            ...
            sub_tool = TOOL_HANDLERS.get(tool_call["name"])
            if not sub_tool:
                raise ValueError(f'工具名称不匹配: {tool_call["name"]}')

            output = sub_tool(**tool_call["args"])
            print(output[:200])
            sub_messages.append(ToolMessage(content=output[:200], tool_call_id=tool_call['id']))
...
TOOL_HANDLERS: Dict[str, Callable[..., Any]] = {
    ...
    "spawn_subagent": lambda **kwargs: spawn_subagent.invoke(kwargs),
}

tools = [run_bash, read_file, write_file, update_todo, spawn_subagent]
...
if __name__ == '__main__':
    parent_messages: List[BaseMessage] = [
        SystemMessage(content=f"您是 {WORK_DIR} 的一名编码员。使用待办事项工具规划多步骤任务,然后交给子Agent执行。开始前标记为“进行中”，完成后标记为“已完成”。建议优先使用工具而非文字描述。")
    ]
    while True:
        try:
            query = input("输入:")
        ...
        if query.strip().lower() in ("q", "exit", ""):
            break
        parent_messages.append(HumanMessage(content=query))
        agent_loop(parent_messages)
        print()
